...
def write_chunks_to_db(reader, engine, table_name="TEMP_TEXT_TEST", if_exists="replace"):
    ...
    for i, chunk in enumerate(reader):
        ...
        selected_columns = ['num_1', 'num_2', 'str_1', 'str_2']  # 指定要写入的列

        # 写入数据库
        chunk[selected_columns].to_sql(
            name=table_name,
            con=engine,
...
if __name__ == "__main__":
    ...
    # 2. 生成大型 TXT 文件
    txt_file = create_sample_txt(num_rows=100000)  # 生成 15102020 行数据

    # 3. 分块读取
    chunk_reader = read_txt_in_chunks(txt_file, chunk_size=10000)  # 每块 10000 行
