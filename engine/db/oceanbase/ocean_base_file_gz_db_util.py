...
class OceanBaseFileGZDbUtil:
    ...
    @staticmethod
    def get_engine():
        if OceanBaseFileGZDbUtil._engine is None:
            # 1. 配置参数
            ...
            ocean_base_pool_size = config.get_config_value("read_tool.oceanbase.app.file_gz.pool_size")


            log.info('获取到的连接CONN_URL=%s', CONN_URL)
            OceanBaseFileGZDbUtil._engine = create_engine(
                CONN_URL,
        ...
        return OceanBaseFileGZDbUtil._engine

    @staticmethod
    def get_session():
        if OceanBaseFileGZDbUtil._session_maker is None:
            OceanBaseFileGZDbUtil._session_maker = sessionmaker(bind=OceanBaseFileGZDbUtil.get_engine())
        return OceanBaseFileGZDbUtil._session_maker()

    @staticmethod
    def execute_dml_sql(sql, params=None):
        with OceanBaseFileGZDbUtil.get_session() as session:
            try:
    ...
    def execute_query_sql_by_context_instance(exec_sql, context_instance):
        ...
        params = context_instance.gen_simple_context_dict()
        sql = replace_sql(exec_sql, context_instance)
        # log.info('替换后的sql是:%s',sql)
        df = OceanBaseFileGZDbUtil.execute_auto_query_sql(sql, params)
        # log.info('执行后的df值是:%s',df)
        return df
    ...
    def execute_auto_query_sql(sql, params=None):
        ...
        with OceanBaseFileGZDbUtil.get_session() as session:
            ...
            column_names = [item.upper() for item in column_names]
            all_data = result.fetchall()
            log.info('执行了OceanBaseFileGZDbUtil.execute_auto_query_sql')
            df = pd.DataFrame(all_data, columns=column_names)
            return df
