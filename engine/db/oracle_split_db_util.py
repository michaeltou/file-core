...
class OracleSplitConnectionPool:
    """
    Oracle 连接池，
    ...
    @staticmethod
    def get_engine():
        if OracleSplitConnectionPool._engine is None:
            # Oracle配置
            ...
            OracleSplitConnectionPool._engine = create_engine(
        ...
        return OracleSplitConnectionPool._engine

    @staticmethod
    def get_session():
        if OracleSplitConnectionPool._session_maker is None:
            OracleSplitConnectionPool._session_maker = sessionmaker(bind=OracleSplitConnectionPool.get_engine())
        return OracleSplitConnectionPool._session_maker()





def execute_split_query_sql(sql, params=None):
    """ 
    :param sql: 
    :param params: 
    :return: 
...
def execute_split_dml_sql(sql, params=None):
    with OracleSplitConnectionPool.get_session() as session:
        try:
...
def test_query_sql():
    ...
    df = execute_split_query_sql(sql, params)
    print(df)
...
def test_dml_sql():
    ...
    execute_split_dml_sql(sql, params)
