...
def get_global_engine():
    ...
    if _global_engine is None:
        _global_engine = OracleConnectionPool.get_engine()
...
class OracleConnectionPool:
    ...
    def get_engine():
        if OracleConnectionPool._engine is None:
            ...
            oracle_mode = config.get_config_value("read_tool.oracle.mode")
            connect_url = config.get_config_value("read_tool.oracle.connect_url")
...
def execute_ddl_sql(sql, params=None):
    with OracleConnectionPool.get_session() as session:
        try:
...
def test_query_sql():
    ...
    df = execute_query_sql(sql, params)
    print(df)
