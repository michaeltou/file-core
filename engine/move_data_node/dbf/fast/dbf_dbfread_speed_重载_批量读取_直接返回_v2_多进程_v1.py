...
def task(record_raw_list, encoding,fields):
    ...
    print('进程id ', os.getpid(), '开始处理数据:',
           'encoding:', encoding, 'fields:', fields)
...
class MyDbf5(Dbf5):
    ...
    def _get_recs_direct_list(self, chunk=None):
        ...
        end_time = time.time()
        print("把读取的文件通过struct.unpack转成raw原始数据列表，耗时",end_time - start_time,"秒")
        ...
        # 均匀分割列表
        splits = np.array_split(raw_record_list, process_count)
        my_encoding = self._enc
        my_fields = self.fields
        splits = [(split_item,my_encoding, my_fields) for split_item in splits]  # 转回普通列
        print('构造数据列表，耗时：', time.time() - start_time, '秒')
        # splits = [(split_item, my_encoding, my_fields) for split_item in range(4)]  # 转回普通列


        # 创建包含4个工作进程的进程池
        with Pool(processes=process_count) as pool:
            # 提交任务到进程池
            result_list_for_multi_process = pool.starmap(task, splits)
        ...
        end_time = time.time()
        print("通过多进程解析数据，总共耗时{}秒".format(end_time - start_time))
