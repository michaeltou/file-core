...
class MyDbf5(Dbf5):
    ...
    def _get_recs_direct_list(self, chunk=None):
        ...
        for record in raw_record_list:
            ...
            final_record = self.task(record)
            # 创建包含4个工作线程的线程池
            # with ThreadPoolExecutor(max_workers=4) as executor:
            #     # 提交任务到线程池
            #     futures = [executor.submit(self.task, record) for i in range(my_chunk_size)]
            #
            #     # 获取结果
            #     results = [f.result() for f in futures]

            # 这里直接把结果放到列表里，不用yield，加快速度
            final_record_list.append(final_record)
        end_time = time.time()
